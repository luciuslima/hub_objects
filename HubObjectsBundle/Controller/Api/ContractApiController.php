...
use MauticPlugin\HubObjectsBundle\Form\Type\ContractType;
use MauticPlugin\HubObjectsBundle\Model\ContractModel;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\HttpKernel\Event\FilterControllerEvent;

class ContractApiController extends CommonApiController
{
    /**
     * @var ContractModel
     */
    ...
    {
        $this->model = $this->factory->getModel('hubobjects.contract');
    ...
    public function getEntitiesAction(Request $request): Response
    {
        ...
        $view = $this->view(
            ['contracts' => $entities, 'total' => $count],
            200,
            [],
            ['serializerGroups' => ['contractList']]
        );
    ...
    public function getEntityAction(Request $request, $id): Response
    {
        ...
        $view = $this->view(['contract' => $entity], 200, [], ['serializerGroups' => ['contractDetails']]);
    ...
    public function newEntityAction(Request $request): Response
    {
        ...
        $form   = $this->get('form.factory')->create(ContractType::class, $entity);

        $form->submit($request->request->all());

        if ($form->isSubmitted() && $form->isValid()) {
            $this->model->saveEntity($entity);
            $view = $this->view(['contract' => $entity], 201, [], ['serializerGroups' => ['contractDetails']]);
        } else {
    ...
    public function editEntityAction(Request $request, $id, $createIfNotExists = false): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $view       = $this->view(['contract' => $entity], $statusCode, [], ['serializerGroups' => ['contractDetails']]);
        } else {
            return $this->returnError($this->getFormErrorMessages($form), 400);
        }
    ...
    public function deleteEntityAction(Request $request, $id): Response
    {
        ...
        $view = $this->view(['contract' => $entity]);
